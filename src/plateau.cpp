...
bool peutRelancer(int desObtenue[NB_DES], int desGarder[NB_DES], int desEnJeu) {
    ...
    for (int i = 0; i < desEnJeu; i++) {
        bool dejaGardee = std::any_of(desGarder, desGarder + NB_DES, [desObtenue, i](int v) { return v == desObtenue[i]; });
        if (!dejaGardee)
            return true;
    }
    return false;
...
void garderDES(int desGarder[NB_DES], int desObtenue[NB_DES], int desEnJeu) {
    ...
    do {
        ...
        if (valeurGardee == -1) { afficherErreurEntree(); return; }

        dejaGardee = std::any_of(desGarder, desGarder + NB_DES, [valeurGardee](int v) { return v == valeurGardee; });
        if (dejaGardee) afficherValeurDejaGardee();
